...
function [pas] = holographic_stereogram_pas(lightField, lightFieldDepth, k)
    ...
    lightFieldDepth = padarray(lightFieldDepth, [0 0 cropRadius(1,:)], 'pre');
    lightFieldDepth = padarray(lightFieldDepth, [0 0 cropRadius(2,:)], 'post');
    ...
    pas = zeros([hologramResolution numColorChannels]); 
    
    hc = hogelCenter; hr = hogelResolution;
    complexLF = sqrt(lightField(:,:,hc(1):hr(1):end,hc(2):hr(2):end,:));
    depthWaves = zeros(size(complexLF));
    for c = 1:numColorChannels
        depthWaves(:,:,:,:,c) = k(c) .* lightFieldDepth(:,:,hc(1):hr(1):end,hc(2):hr(2):end);
    end
    complexLF = complexLF .* exp(1i .* depthWaves);
    ...
    for ky = 1:hogelResolution(1)
        for kx = 1:hogelResolution(2)
            pas(ky:hogelResolution(1):end, kx:hogelResolution(2):end, : ) =...
                squeeze(complexLF(ky, kx, :, :, :));
        end
    end

    % crop back to original light field size
    pas = pas(1+cropRadius(1,1):end-cropRadius(2,1),...
